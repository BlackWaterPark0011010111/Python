...
в теке есть своя цена- при сохранении всей промежуточной информации может привести к весомым затратам памяти. и в этом случае-либо переписывать  код на замену с циклом,либо иногда использовать хвостовую рекурсию.

рекурсивная- функция вызывает саму себя
и в ней 2 случая одновременно-базовый и рекурсивный.
стек -2 операции:занесение и извлечение.
все вызовы функций сохраняются в стеке памяти вызовов.

Быстрая сортировка
сначала определяется базовый случай решения-самый простейший из всех возможных.задача делится или сокращается до тех пор,пока не будет сведена к базовому случаю.например,какой самый большой размер  квадрата  может использоваться  в прямоугольнике?.
1-е  -- длина одной стороны кратна длине другой.
одна сторона составляет 25,а другая 50. то при самом большом размере участка это будет 25х25, и нарез после деления будет состоять из двух участков.
тут при каждом рекурсивном случае ,задача должна сокращаться. при размере640х1680. получится 2 учатска по 640 и остаток 400 х 640(нераспределенный участок )
остаток-тоже нужно разделить. и так,при нахождении самого большого участка -это будет самый подходящий,и сейчас сокращение получилось с 1680х640 до 640х400.
TODO: "алгоритм Евклида"- при нахождении самого большого участка ,подходящего для этого размера,это и будет самый большой участок,подходящий для всей фермы.
при начале работы с участком 640х400,то размеры самого большого будут 400х400,и останется самый меньший 400х240. отсекая поделенную часть,мы переходим к еще меньшей сетке 240х240 и 160. после остается 160х160 + остаток 80.
80х160=80х80х80.
Для исходного разделения земли самый большой размер участка будет равен 80х80!. чтобы все деления земли были равны.
...
в любом случае результат 12,но во втором я сократила размер массива и соответственно то есть размер задачи.
При написании рекурсивной функции в которой есть массив,то базовым случаем оказывается пустой массив из одного элемента.
в функциональном языке Haskell,циклов нет и для написания подобного нужна рекурсия
глубина рекурсии ограничена-для больших списков возникнет RecursionError.
...
сортировка массива из 3х элементов
10  15  7
    |
    v
7  10  15

массив из 5-ти элементов:
...
qsort(2 1) |3| qsort(4 5)
   ...
   1 2 |3| 4 5
        ...
        v
    1 2 3 4 5
Это доказательство по индукции. оно состоит из двух частей: базы (базового случая ) и индукционного перехода

О-большое

![alt text](download.jpg)

здесь представление по  оценке времени при выполнении компьютером 10-ти операций в секунду.
При использовании О большого,например (О(n)),то в действительности это означает: С*n
С- некоторый фиксированный промежуток  времени моего алгоритма==Константа. Обычно константа игнорируется,потому что если 2 алгоритма имеют разное время О-большое,она роли не играет.
К примеру, ведется поиск из 4 миллиардов элементов,то время будет таким:

ПРОСТОЙ ПОИСК --- 10мс * 4 миллиарда == 463 дня
БИНАРНЫЙ ПОИСК --- 1с *32 == 32 секунды

Бинарный поиск быстрее и константа не повлияла.
но может влиять на быструю сортировку и сортировку слиянием. у быстрой константа меньше  и поэтому не смотря  на то что оба алгоритма характеризуются  временем О(n log n),быстрая сортировка быстрее.
Быстродействие быстрой сортировки зависит от выбора опорного элемента. и если опорным элементом выбран первый элемент,а быстрая применяется к уже отсортированному массиву. Быстрая сортировка не проверяет отсортирован входной массив или нет и все равно его сортирует. К примеру если взять 8 элементов и опорный будет --1. то проход будет от 1 и до 8-рки,то есть общая высота вызова стека будет 8. а если выбрать средний,то размер  стека вызовов будет 4 
1 2 3 4 5 6 7 8           |                    
        |                 |    
        v                 |  размер стека вызовов:
1 2 3  |4|   5 6 7 8      |          4
 |              |         |
 v              v         |   высота вызова стека ==
 1 |2| 3      |6| 7 8     |         O(log n)
                  |       |     
                  v       |     
                [] |7| 8  |       

стек короче а массив каждый раз делится надвое
\
И если разделять массив другим способом,то все равно обращаемся к O(n) элементам.
Завершение каждого уровня - это  требует O(n) времени.

[[лучший случай также является средним. Если вы всегда будете выбирать опорным элементом случайный элемент в массиве, быстрая сортировка в среднем завершится за время O(n log n). Это один из самых быстрых существующих алгоритмов сортировки, который заодно является хорошим примером стратегии «разделяй и властвуй». ]]
